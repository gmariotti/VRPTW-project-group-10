...
/**
 * This version of the {@link TabuSearch} does not create any new threads, making it ideal for
 * embedding in Enterprise JavaBeans. The {@link #startSolving} method blocks until the given number
 ...
 * The Common Public License, developed by IBM and modeled after their industry-friendly IBM Public
 * License, differs from other common open source licenses in several important ways:
 * <ul>
 * <li>You may include this software with other software that uses a different (even non-open
 * source) license.</li>
...
public class SingleThreadedTabuSearch extends TabuSearchBase {
	...
	/**
	 * Constructs a <tt>SingleThreadedTabuSearch</tt> with all tabu objects set. The initial
	 * solution is evaluated with the objective function, becomes the <tt>currentSolution</tt> and a
	 * copy becomes the <tt>bestSolution</tt>.
	 * 
	 * @param initialSolution
	 *            The initial <tt>currentSolution</tt>
	 * @param moveManager
	 *            The move manager
	 ...
	 *            The aspiration criteria or <tt>null</tt> if none is to be used
	 * @param maximizing
	 ...
	 * @since 1.0
	 */
	public SingleThreadedTabuSearch(Solution initialSolution, MoveManager moveManager,
			ObjectiveFunction objectiveFunction, TabuList tabuList,
			AspirationCriteria aspirationCriteria, boolean maximizing) {
		this();
	 ...
	 * @since 1.0
	 */
	protected void performOneIteration() throws NoMovesGeneratedException,
		...
		final Object[] bestMoveArr = getBestMove(currentSolution, moves, objectiveFunction,
				tabuList, aspirationCriteria, maximizing, chooseFirstImproving);
	 ...
	 * moves.
	 * 
	 * It's not static so that when the MultiThreadedTabuSearch invokes the performOneIteration
	 * method the proper method is invoked. Java's weird about overriding static methods...
	 * 
	 * @since 1.0
	 */
	protected Object[] getBestMove(final Solution soln, final Move[] moves,
		...
		return SingleThreadedTabuSearch.getBestMove(soln, moves, objectiveFunction, tabuList,
				aspirationCriteria, maximizing, chooseFirstImprovingMove, getIterationsCompleted(),
				this);
	} // end getBestMove
	...
	protected static Object[] getBestMove(final Solution soln, final Move[] moves,
			final ObjectiveFunction objectiveFunction, final TabuList tabuList,
			final AspirationCriteria aspirationCriteria, final boolean maximizing,
			final boolean chooseFirstImprovingMove, final int iteration, final TabuSearch This) {
		// Set up variables
		...
		bestMoveTabu = moves.length == 0 ? false : // Don't bother calling the tabu list if there's
													// only one move.
		...
		for (int i = 1; i < movesLen; i++) {
			...
			for (i = 1; i < moves.length; i++) {
				...
				if (isFirstBetterThanSecond(newObjVal, bestMoveVal, maximizing)) {
					...
					boolean newIsTabu = isTabu(soln, move, newObjVal, tabuList, aspirationCriteria,
							iteration, This);
					...
					if (!(!bestMoveTabu && newIsTabu)) {
						...
						if (chooseFirstImprovingMove)
							if (!bestMoveTabu
									&& isFirstBetterThanSecond(bestMoveVal, currSolnVal, maximizing))
								return new Object[] { bestMove, bestMoveVal,
										new Boolean(bestMoveTabu) };
						...
						// boolean newIsTabu;
					if (bestMoveTabu
							&& !isTabu(soln, move, newObjVal, tabuList, aspirationCriteria,
									iteration, This)) {
						bestMove = move;
						bestMoveVal = newObjVal;
						bestMoveTabu = false;
	...
	/**
	 * Determine if the move is tabu and consider whether or not it satisfies the aspiration
	 * criteria.
	 * 
	 * @since 1.0
	 */
	protected static boolean isTabu(final Solution soln, final Move move, final double[] val,
			...
			final int iterations, final TabuSearch This) {
		boolean tabu = false;
		// See if move is tabu
		if (tabuList.isTabu(soln, move, iterations)) { // It is tabu.
			tabu = true;
	...
	/**
	 * Deprecated and renamed to {@link #isFirstBetterThanSecond} to be named more consistently.
	 * This method still works. It simply calls the newly-named version.
	 * 
	 * @param first
	 *            The first array of <tt>double</tt>s
	 * @param second
	 *            The second array of <tt>double</tt>s
	 * @param maximizing
	 ...
	 * @deprecated
	 */
	public static boolean firstIsBetterThanSecond(final double[] first, final double[] second,
	 ...
	 * 
	 * @param solving
	 *            Whether or not the tabu search should be marked as solving or not.
	 * @since 1.0
	 */
	...
	/**
	 * Internally set whether or not a new current solution {@link TabuSearchEvent} should be fired
	 * at the end of the iteration.
	 * 
	 * @param b
	 *            Whether or not to fire a new current solution event.
	 * @since 1.0
	 */
	 ...
	 * 
	 * @param b
	 *            Whether or not to fire a new best solution event.
	 * @since 1.0
	 */
	...
	/**
	 * Internally set whether or not an unimproving move made {@link TabuSearchEvent} should be
	 * fired at the end of the iteration.
	 * 
	 * @param b
	 *            Whether or not to fire an unimproving move made event.
	 * @since 1.0
	 */
	...
	/**
	 * Internally set whether or not an improving move made {@link TabuSearchEvent} should be fired
	 * at the end of the iteration.
	 * 
	 * @param b
	 *            Whether or not to fire an improving move made event.
	 * @since 1.0-exp7
	 */
	...
	/**
	 * Internally set whether or not a no change in value move made {@link TabuSearchEvent} should
	 * be fired at the end of the iteration.
	 * 
	 * @param b
	 *            Whether or not to fire a no change in value move made event.
	 * @since 1.0-exp7
	 */
	...
	/**
	 * Sets the objective function, effective at the start of the next iteration, and re-evaluates
	 * the current and best solution values.
	 * 
	 * @param function
	...
	/**
	 * Sets the aspiration criteria, effective at the start of the next iteration. A <tt>null</tt>
	 * value means there is no aspiration criteria.
	 * 
	 * @param aspirationCriteria
	 *            The new aspiration criteria
	 * @see AspirationCriteria
	 * @since 1.0
	 */
	...
	/**
	 * Sets the best solution, effective at the start of the next iteration, and fires an event to
	 * registered {@link TabuSearchListener}s.
	 * 
	 * @param solution
	 *            The new best solution.
	 * @see Solution
	...
	/**
	 * Sets the current solution, effective at the start of the next iteration, and fires an event
	 * to registered {@link TabuSearchListener}s.
	 * 
	 * @param solution
	 *            The new current solution.
	 * @see Solution
	...
	/**
	 * Sets the number of iterations that the tabu search has left to go. If the tabu search was
	 * previously idle, that is <tt>iterationsToGo</tt> was less than or equal to zero, the tabu
	 * search will not automatically begin again. In this case the tabu search will not begin again
	 * until {@link #startSolving} is called.
	 * 
	 * @param iterations
	 *            The number of iterations left for the tabu earch to execute.
	 * @see #startSolving
	 * @since 1.0
	 */
	...
	/**
	 * Sets whether the tabu search should be maximizing or minimizing the objective function. A
	 * value of <tt>true</tt> means <em>maximize</em>. A value of <tt>false</tt> means
	 * <em>minimize</em>.
	 * 
	 * @param maximizing
	 *            Whether or not the tabu search should be maximizing the objective function.
	...
	/**
	 * Setting this to <tt>true</tt> will cause the search to go faster by not necessarily
	 * evaluating all of the moves in a neighborhood for each iteration. Instead of evaluating all
	 * of the moves and selecting the best one for execution, setting this will cause the tabu
	 * search engine to select the first move that it encounters that causes an improvement to the
	 * current solution. The default value is <tt>false</tt>.
	 * 
	 * @param choose
	...
	/**
	 * Returns the aspiration criteria. A <tt>null</tt> value means there is no aspiration criteria.
	 * 
	...
	/**
	 * Returns whether or not the tabu search engine will choose the first improving move it
	 * encounters at each iteration (<tt>true</tt>) or the best move (<tt>false</tt>).
	 * 
	 * @since 1.0.1
	 */
